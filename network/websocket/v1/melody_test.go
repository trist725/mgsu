...
func NewTestServerHandler(handler handleMessageFunc) *Server {
	m := &Server{
		Melody: NewMelody(DefaultConfig),
	}
	m.HandleTextMessage(handler)
	return m
}
...
func TestWriteClosed(t *testing.T) {
	...
	fn := func(msg string) bool {
		...
		echo.HandleConnect(func(s *Session) {
			s.Close()
		})

		echo.HandleDisconnect(func(s *Session) {
			err := s.WriteText([]byte("hello world"))
...
func TestEchoBinary(t *testing.T) {
	echo := NewTestServer()
	echo.HandleBinaryMessage(func(session *Session, msg []byte) {
...
func TestHandlers(t *testing.T) {
	echo := NewTestServer()
	echo.HandleTextMessage(func(session *Session, msg []byte) {
		session.WriteText(msg)
	})
	server := httptest.NewServer(echo)
	defer server.Close()
	...
	echo.HandleConnect(func(session *Session) {
		q = session
		session.Close()
	})

	echo.HandleDisconnect(func(session *Session) {
		if q != session {
...
func TestMetadata(t *testing.T) {
	...
	echo.HandleTextMessage(func(session *Session, msg []byte) {
		...
		session.WriteText([]byte(strconv.Itoa(int(stamp))))
	})
	server := httptest.NewServer(echo)
	defer server.Close()
...
func TestUpgrader(t *testing.T) {
	...
	broadcast.HandleTextMessage(func(session *Session, msg []byte) {
		session.WriteText(msg)
	})
	...
	broadcast.HandleError(func(session *Session, err error) {
...
func TestBroadcastBinary(t *testing.T) {
	broadcast := NewTestServer()
	broadcast.HandleBinaryMessage(func(session *Session, msg []byte) {
		broadcast.BroadcastBinary(msg)
	})
	server := httptest.NewServer(broadcast)
	defer server.Close()
...
func TestBroadcastOthers(t *testing.T) {
	broadcast := NewTestServer()
	broadcast.HandleTextMessage(func(session *Session, msg []byte) {
		broadcast.BroadcastTextOthers(msg, session)
	})
...
func TestBroadcastBinaryOthers(t *testing.T) {
	broadcast := NewTestServer()
	broadcast.HandleBinaryMessage(func(session *Session, msg []byte) {
		broadcast.BroadcastBinaryOthers(msg, session)
	})
	broadcast.Config.PongWait = time.Second
	broadcast.Config.PingPeriod = time.Second * 9 / 10
	server := httptest.NewServer(broadcast)
...
func TestBroadcastFilter(t *testing.T) {
	broadcast := NewTestServer()
	broadcast.HandleTextMessage(func(session *Session, msg []byte) {
		broadcast.BroadcastTextFilter(msg, func(q *Session) bool {
			return session == q
		})
	})
	server := httptest.NewServer(broadcast)
...
func TestBroadcastBinaryFilter(t *testing.T) {
	broadcast := NewTestServer()
	broadcast.HandleBinaryMessage(func(session *Session, msg []byte) {
		broadcast.BroadcastBinaryFilter(msg, func(q *Session) bool {
			return session == q
		})
	})
	server := httptest.NewServer(broadcast)
...
func TestSmallMessageBuffer(t *testing.T) {
	...
	echo.HandleError(func(s *Session, err error) {
		if err == nil {
...
func TestPong(t *testing.T) {
	...
	fired := false
	echo.HandlePong(func(s *Session) {
		fired = true
	})
