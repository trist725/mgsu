	...
	for {
		...
		case msg, ok := <-s.sendQueue:
			...
			if msg.t == websocket.TextMessage {
				s.melody.afterSendTextMessageHandler(s, msg.msg)
			}

			if msg.t == websocket.BinaryMessage {
				s.melody.afterSendBinaryMessageHandler(s, msg.msg)
			}
		case <-ticker.C:
			s.ping()
		}
...
func (s *Session) readPump() {
	...
	for {
		...
		if t == websocket.TextMessage {
			s.melody.textMessageHandler(s, message)
		}

		if t == websocket.BinaryMessage {
			s.melody.binaryMessageHandler(s, message)
		}
	}
}
